...
import type { CheckResult, TranslationIssue } from './types.js';
...
export class TranslationChecker {
  ...
  private filesChecked = new Set<string>();

  async checkProject(projectPath: string = process.cwd()): Promise<CheckResult> {
  ...
  private hasUseTranslation(content: string): boolean {
    return /useTranslation/.test(content);
  }

  private checkLineForTranslations(line: string, lineNumber: number, filePath: string): void {
